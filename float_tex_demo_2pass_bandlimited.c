...
int main(void)
{
    ...
    // 生成 mipmap 链，用于后续的带限滤波（polar twirl 抗锯齿）
    GenTextureMipmaps(&fbo.texture);
    SetTextureFilter(fbo.texture, TEXTURE_FILTER_TRILINEAR);
    ...
    const char *fsPass2 =
        ...
        "        // Band-limiting #1: 用 sin 使扭曲周期性振荡，避免角度无限累积\n"
        "        float twist = sin(uTime * 0.4) * 2.5 * (1.0 - radius);\n"
        "        angle += twist;\n"
        "        vec2 twirl = vec2(cos(angle), sin(angle)) * radius + 0.5;\n"
        "        // Band-limiting #2: 利用 dFdx/dFdy 计算梯度，自动选择合适 mip level\n"
        "        vec2 tx = dFdx(twirl);\n"
        "        vec2 ty = dFdy(twirl);\n"
        "        float lod = 0.5 * log2(max(dot(tx, tx), dot(ty, ty)));\n"
        "        c = textureLod(texture0, twirl, lod);\n"
        "    }\n"
        "    // ----- 效果 5: CRT 模拟 (所有效果大杂烩) -----\n"
        "    else\n"
        "    {\n"
